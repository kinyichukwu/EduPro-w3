...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      signOut: async () => {
        set({ loading: { ...get().loading, signOut: true }, error: null });
        try {
          ...
          set({
            user: null,
            error: null,
            loading: { ...get().loading, signOut: false },
          });
      ...
      updateOnboarding: async (onboardingData: OnboardingData) => {
        ...
        set({
          loading: { ...get().loading, updateOnboarding: true },
          error: null,
        });
        ...
        try {
          ...
          const backendOnboardingData: Partial<ApiOnboardingData> = {
            ...
            custom_learning_goal: onboardingData.customLearningGoal,
            academic_details: onboardingData.academicDetails
              ? {
                  ...
                  ...(onboardingData.role === "jamb" &&
                    onboardingData.academicDetails.preferredUniversity && {
                      jamb_details: {
                        preferred_university:
                          onboardingData.academicDetails.preferredUniversity,
                        preferred_course:
                          onboardingData.academicDetails.preferredCourse ?? "",
                        target_score:
                          onboardingData.academicDetails.targetScore,
                        jamb_year: onboardingData.academicDetails.jambYear,
                        jamb_subjects:
                          onboardingData.academicDetails.jambSubjects ?? [],
                      },
                    }),
                  ...(["undergraduate", "university", "masters"].includes(
                    onboardingData.role
                  ) &&
                    onboardingData.academicDetails.university && {
                      university_details: {
                        current_university:
                          onboardingData.academicDetails.university,
                        current_course:
                          onboardingData.academicDetails.course ?? "",
                        current_level: onboardingData.academicDetails.level,
                        matric_number:
                          onboardingData.academicDetails.matricNumber,
                      },
                    }),
                  ...(onboardingData.role === "lecturer" &&
                    onboardingData.academicDetails.university && {
                      lecturer_details: {
                  ...
                  ...(onboardingData.role === "custom" &&
                    onboardingData.customLearningGoal && {
                      custom_details: {
                        learning_goal: onboardingData.customLearningGoal,
                        education_level:
                          onboardingData.academicDetails.educationLevel,
                        experience_level:
                          onboardingData.academicDetails.experienceLevel,
                        additional_details:
                          onboardingData.academicDetails.additionalDetails,
                      },
                    }),
                }
              : undefined,
          ...
          const response = await apiService.updateOnboarding(
            backendOnboardingData
          );
        ...
        try {
          ...
          if (response.data) {
            ...
            if (backendUser.onboarding_data) {
              ...
              onboardingData = {
                role: bd.role,
                customLearningGoal: bd.custom_learning_goal,
                academicDetails: bd.academic_details
                  ? {
                      ...
                      // JAMB details
                      preferredUniversity:
                        bd.academic_details.jamb_details?.preferred_university,
                      preferredCourse:
                        bd.academic_details.jamb_details?.preferred_course,
                      targetScore:
                        bd.academic_details.jamb_details?.target_score,
                      jambYear: bd.academic_details.jamb_details?.jamb_year,
                      jambSubjects:
                        bd.academic_details.jamb_details?.jamb_subjects,
                      // University details
                      level:
                        bd.academic_details.university_details?.current_level,
                      matricNumber:
                        bd.academic_details.university_details?.matric_number,
                      // Lecturer details
                      experience:
                        bd.academic_details.lecturer_details?.experience,
                      title:
                        bd.academic_details.lecturer_details?.academic_title,
                      // Custom details
                      educationLevel:
                        bd.academic_details.custom_details?.education_level,
                      experienceLevel:
                        bd.academic_details.custom_details?.experience_level,
                      additionalDetails:
                        bd.academic_details.custom_details?.additional_details,
                    }
                  : undefined,
                completedAt: bd.completed_at
                  ? new Date(bd.completed_at)
                  : undefined,
              };
            }
