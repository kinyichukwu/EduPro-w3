...
// Pass-through behavior: send the user's query as-is (no custom prompt/formatting)
func (c *Client) GenerateExplanation(req *GeminiRequest) (*models.ExplanationResponse, error) {
	logger := utils.GetLogger()

	// Sanitize inputs (but do not alter the meaning or wrap with templates)
	topic := SanitizeInput(req.Query)
	subject := SanitizeInput(req.Subject)
	level := SanitizeInput(req.Level)
	...
	logger.Info("Generating explanation (pass-through)",
		zap.String("topic", topic),
		zap.String("subject", subject),
		zap.String("level", level),
	)
	...
	// Build response: fill only explanation; keep other fields optional/empty
	...
	logger.Info("Explanation generated successfully (pass-through)",
		zap.String("topic", topic),
	)
